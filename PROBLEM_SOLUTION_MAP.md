...
| P08 | Reference matrix вокруг текущей гипотезы закрепляет ошибку | Частично смягчено | `main.py`, `correlator.py`, top-k + physical gates | Добавлена защита от физически невозможных фиксов и возможность выбрать сильную top-k альтернативу. Но фундаментально нужен 2D/multi-hypothesis search, а не одна стартовая точка. |
