...
def test_iter_case_unified_samples_merges_case_inputs(tmp_path: Path) -> None:
    ...
    assert samples[1]["nav_mode"] == "INIT"


def test_iter_case_unified_samples_rejects_radar_timestamp_drift(tmp_path: Path) -> None:
    radar_path = tmp_path / "radar_data.nmea"
    truth_path = tmp_path / "truth.csv"
    baro_path = tmp_path / "barometer.csv"
    dem_path = tmp_path / "terrain.tif"
    dem_path.write_text("stub", encoding="utf-8")
    radar_path.write_text(
        _gpgga_line("123519.000", 1200.0) + _gpgga_line("123520.500", 1198.5),
        encoding="ascii",
    )
    truth_path.write_text(
        "\n".join(
...
def test_iter_case_unified_samples_accepts_midnight_rollover(tmp_path: Path) -> None:
    radar_path = tmp_path / "radar_data.nmea"
    truth_path = tmp_path / "truth.csv"
    baro_path = tmp_path / "barometer.csv"
    dem_path = tmp_path / "terrain.tif"
    dem_path.write_text("stub", encoding="utf-8")
    radar_path.write_text(
        _gpgga_line("235959.900", 1200.0) + _gpgga_line("000000.100", 1198.5),
        encoding="ascii",
    )
    truth_path.write_text(
        "\n".join(
